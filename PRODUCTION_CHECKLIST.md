# Production acceptance checklist — 2.1.1
...
|---|---|---|---|---|---|
| P01 | Workflow syntax and expressions | `npm run ci:validate` | actionlint: 3/3 workflow; 33 action references; static validation and action tag verification PASS | текущий release-прогон | PASS |
...
| P03 | Dependency install | `npm ci` | 409 packages installed; зависимости не изменены; lockfile отличается от исходного только двумя корневыми полями версии | текущий release-прогон | PASS |
...
| P15 | Dockerfile | static validation | Node 24.18.1 bookworm-slim и архитектура сохранены; оба npm cache mount используют `sharing=locked`; retry/timeout-параметры присутствуют; production сохраняет `--omit=dev --ignore-scripts` | `Dockerfile` | PASS_STATIC |
...
| P17 | Docker build/runtime/Trivy | `docker compose build`, `docker compose up -d`, `docker compose ps/logs`, повторный build | команды запущены, но Docker CLI отсутствует (`exit 127`); production smoke без контейнеров получил `ECONNREFUSED` | `RELEASE_REPORT_v2.1.1.md` | NOT_RUN_DOCKER |
...
| P20 | Final ZIP recheck | unpack + mandatory clean cycle | CRC/root/exclusions/unpack проверяются перед передачей; Docker build распакованного ZIP недоступен без Docker CLI | `RELEASE_REPORT_v2.1.1.md` | BLOCKED_DOCKER |
