...
- Docker BuildKit npm-cache mounts для build и production dependencies используют `sharing=locked`.
- Оба `npm ci` выполняются с последовательным скачиванием, повторными попытками и увеличенным сетевым тайм-аутом; `--omit=dev --ignore-scripts` production-стадии сохранены.
- Версионные метаданные приложения согласованно обновлены до 2.1.1. Зависимости, алгоритмы, бизнес-логика, API, интерфейс и функциональность не изменялись.

## 2.1.0 — 2026-08-04

- Удалён справочный выбор нормы расхода: калькулятор ЛКМ использует ручной ввод нормы и сохраняет передачу окрашиваемой площади из CAD.
- Для новых и сохранённых CAD-расчётов постоянно отображаются две основные площади; диагностика и инструменты доступны через блок «Подробнее».
- Ошибки, предупреждения и индикатор `review_required` остаются видимыми при закрытом блоке; печатная версия содержит полный отчёт.
- Актуализированы эксплуатационные документы и release-проверки. Устаревшие ссылки на удалённые функции отсутствуют в актуальной документации и env-примерах.
- Golden, regression и determinism baseline сохранён без обновления эталонов; CAD-алгоритмы, API, SQLite, история расчётов и PaintIntegration не изменялись.

## Этап 7 — 2026-08-04

- В сохранённом CAD-расчёте Toolbar, 3D Viewer, «Полная площадь» и «Площадь для окрашивания» остаются видимыми сразу после открытия.
- Остальные площади, формула, Feature Rules, Contacts, Features, ручные действия и служебная информация перенесены в закрытый по умолчанию нативный блок «Подробнее».
- Ошибки, предупреждения и количество объектов `review_required` остаются видимыми при закрытом блоке; печать временно раскрывает все вложенные разделы и восстанавливает прежнее состояние.
- CAD-алгоритмы, API, SQLite, история ревизий, отчёты, PaintIntegration и управление сохранёнными расчётами не изменялись.

## Этап 6 — 2026-08-04

- В новом CAD-расчёте постоянно отображаются только «Полная площадь» и «Площадь для окрашивания»; остальные панели доступны в закрытом по умолчанию нативном блоке «Подробнее».
- Ошибки, предупреждения и количество объектов, требующих проверки, остаются видимыми при закрытом блоке.
- Print-стили раскрывают полную диагностическую информацию независимо от экранного состояния блока.
- CAD-алгоритмы, API, SQLite, история расчётов и передача площади в калькулятор ЛКМ не изменялись.

## Этап 5 — 2026-08-04

- Добавлена неблокирующая одноразовая миграция, удаляющая устаревшую браузерную IndexedDB материалов `profigym-user-database`.
- После успешного удаления записывается отдельный версионированный маркер; ошибки и блокировка не мешают запуску приложения и допускают повторную попытку.
- Серверная CAD SQLite, сохранённые расчёты и Docker volumes миграцией не затрагиваются.
