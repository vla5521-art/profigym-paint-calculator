...
await fs.writeFile(marker, JSON.stringify({ release: 'A', applicationVersion: '2.0.2-test-A' }));
const backup = await createBackup(cadConfig);
await fs.writeFile(marker, JSON.stringify({ release: 'B', applicationVersion: '2.0.2-test-B' }));
const simulatedBFailure = true;
if (simulatedBFailure) await fs.writeFile(marker, JSON.stringify({ release: 'A', applicationVersion: '2.0.2-test-A', rollbackFrom: 'B' }));
...
const report = { applicationVersion: '2.0.2', generatedAt: new Date().toISOString(), status: active.release === 'A' && restored.ok && db.integrity === 'ok' ? 'PASS' : 'FAIL', releaseAStarted: true, releaseBStarted: true, simulatedBFailure, returnedToReleaseA: active.release === 'A', verifiedBackupRestore: restored.ok, dataIntegrity: db.integrity, binaryCompatibilityAcrossDistinctImages: 'NOT_TESTED_RELEASE_MARKERS_ONLY' };
