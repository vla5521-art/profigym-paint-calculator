...
await writeJson(path.join(reportsDir,'unit-results.json'),{schemaVersion:'1.0.0',applicationVersion:'2.0.0',generatedAt:new Date().toISOString(),status:'PASS',nodeTestFiles:nodeFiles,frontendTestFiles:frontendFiles,suites:nodeFiles+frontendFiles,nodeTests:node.tests,nodePassed:node.passed,nodeFailed:node.failed,frontendTests:vitest.numTotalTests??0,frontendPassed:vitest.numPassedTests??0,frontendFailed:vitest.numFailedTests??0,note:'Written only after machine-readable node:test and Vitest results both report success.'});
