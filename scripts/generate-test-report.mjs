...
const report = {
  applicationVersion: '2.0.0', testReportSchemaVersion: '1.1.0', generatedAt: new Date().toISOString(), nodeVersion: process.version, os: os.platform(), architecture: os.arch(), gitCommit: process.env.GITHUB_SHA ?? null,
  ...
  environmentDependent: { dockerBuild: dockerAvailable ? 'AVAILABLE_NOT_INFERRED' : 'NOT_RUN_DOCKER_UNAVAILABLE', composeRuntime: dockerAvailable ? 'AVAILABLE_NOT_INFERRED' : 'NOT_RUN_DOCKER_UNAVAILABLE', containerScan: 'NOT_RUN_REQUIRES_DOCKER_OR_CI', githubActions: 'NOT_RUN_NO_REMOTE_RUN_ID', staging: 'NOT_PUBLISHED', production: 'NOT_PUBLISHED', realTls: 'NOT_RUN_NO_DOMAIN', clamavEicar: 'NOT_RUN', firefox: 'NOT_RUN', webkit: 'NOT_RUN' },
...
const testReport = `# TEST_REPORT — PROFiGYM 2.0.0

Автоматически сформирован: ${report.generatedAt}. Итог: **${overallStatus}**.

- Node unit/integration/API: ${report.testCounts.nodeUnitIntegration}; frontend jsdom: ${report.testCounts.frontendJsdom}.
...
const stageStatus = overallStatus === 'FAIL' ? 'FAIL' : 'DEPLOYMENT_READY_NOT_PUBLISHED';
const stage = `# STAGE7_REPORT

Версия: 2.0.0. Итоговый статус: **${stageStatus}**.

## Фактическая архитектура

Nginx proxy → Node app/API → SQLite durable queue → отдельный CAD worker → OCCT/WASM → persistent volumes. Runtime image основан на Node 24.14.0 bookworm-slim, запускается non-root через tini. Image name по умолчанию \`profigym-calculator:sha-local\`; digest отсутствует, поскольку Docker build/push в этой среде не подтверждён.
...
Auth: bearer token или HttpOnly SameSite session; rate limits по категориям; CORS allowlist; CSP/security headers; optional ClamAV profile. CI/CD: сохранён quality workflow, добавлены container and deploy workflows, immutable tags, environment approvals, protected migration/backup and rollback scripts.
