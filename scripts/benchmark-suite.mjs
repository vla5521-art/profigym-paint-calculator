...
await closeCadKernel(); const report={schemaVersion:'1.0.0',applicationVersion:'2.0.4',generatedAt:new Date().toISOString(),nodeVersion:process.version,status:'PASS',measurementNotes:{geometryValidationMs:'Residual calculation time after instrumented contact, feature and viewer stages; area calculation is not separately exposed by OCCT wrapper.',sqliteSaveLoad:'Covered by benchmark:persistence and migration suites.'},results}; await writeJson(path.join(reportsDir,'performance-results.json'),report); console.log(JSON.stringify(report,null,2));
