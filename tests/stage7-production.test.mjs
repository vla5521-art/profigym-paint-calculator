...
async function runtime(name) {
  ...
  return {
    root,
    config: {
      environment: 'test', applicationVersion: '2.0.4', storageRoot: root,
...
test('production auth, rate limit, request ID, CORS and security headers are enforced', async () => {
  ...
  try {
    ...
    const metrics = await fetch(`${base}/metrics`, { headers: { authorization: `Bearer ${token}-metrics` } }); assert.equal(metrics.status, 200); assert.match(await metrics.text(), /profigym_info\{version="2\.0\.4"/);
