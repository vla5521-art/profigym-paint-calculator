...
const results = [];
test.after(async () => { await closeCadKernel(); await writeJson(path.join(reportsDir, 'golden-results.json'), { schemaVersion: '1.0.0', applicationVersion: '2.0.4', suite: 'B-Rep golden', total: results.length, passed: results.filter((r) => r.pass).length, failed: results.filter((r) => !r.pass).length, results }); });
